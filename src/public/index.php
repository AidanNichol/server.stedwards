...
// require_once('./classes/AuthMapper.php');
// require_once('./classes/CpgMapper.php');
// require_once('./classes/StedsMapper.php');
...
// function isDev(){
//   return $_SERVER['DOCUMENT_ROOT']==="/www/sites/steds-server";
// }
// $steds_walkdata = isDev() ? "/Users/aidan/Websites/htdocsC" : "/home/ajnichol/public_html";
//
// define('WALKDATA', str_replace('\\', '/', realpath($steds_walkdata)).'/');
//
// $config['displayErrorDetails'] = true;
// $config['db']['host']   = isDev() ? "/www/sites/kohsteds/data" : "/home/ajnichol/public_html/data";
// $config['db']['user']   = "user";
// $config['db']['pass']   = "password";
// $config['db']['dbname'] = "StEdsWalks.sdb";
// $config['cpg']['host'] =   'www.stedwardsfellwalkers.co.uk';        // Your database server
// $config['cpg']['user'] =   'ajnichol_cop1';        // Your mysql username
// $config['cpg']['pass'] =   'NjqTJ0ArpL';                // Your mysql password
// $config['cpg']['dbname'] =  'ajnichol_cop1';        // Your mysql database name
// $config['auth']['host'] =  isDev() ? "/www/sites/steds-server/src/public/data" : "./data";        // Your mysql database name
// $app = new \Slim\App(["settings" => $config]);
// $container = $app->getContainer();
//
// $container['view'] = new \Slim\Views\PhpRenderer("../templates/");
// $container['couchdb'] = isDev() ? ['http://localhost:5984', 'devbookings'] : ['http://couchdb.nicholware.com', 'bookings'];
// $container['couchdb'] = ['http://www.nicholware.com:5984', 'bookings'];
...
$container['auth'] = function ($c) {
    $db = $c['settings']['auth'];
    $pdo = new PDO("sqlite:{$db['host']}/auth.sqlite");
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
    return $pdo;
};
...
//       $xx = $name . ": " . implode(", ", $values);
//         $this->logger->addInfo("Get getWalkRouteGpx", [$xx]);
//     }
...
$app->get('/walkdata[/{params:.*}]', function (Request $request, Response $response) {
    ...
    $response = $mapper->walkdata($params, $response);
        ...
        // foreach ($headers as $name => $values) {
        //   $xx = $name . ": " . implode(", ", $values);
        //     $this->logger->addInfo("Get walkdata header", [$xx]);
        // }
...
$app->get('/cpg/{method}[/{params:.*}]', function (Request $request, Response $response, $args) {
    ...
    if (!method_exists($mapper, $args['method'])) return $response->withStatus(404);
